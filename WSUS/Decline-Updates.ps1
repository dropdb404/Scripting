...
#        $_.LegacyName -match "ia64" -or #remove updates for itanium computers (1/2)
        ...
        $_.producttitles -match "Vista" -or #remove Windows Vista updates (1/2)
#        $_.producttitles -match "Windows 8" -or #remove Windows 8/8.1 updates (1/2)      (as windows 2008R2 and windows 2012R2 updates also staring with "Windows 8", so can't filter this keyword"
        $_.producttitles -match "Windows 10" -or #remove Windows 10 updates (1/2)      (as windows 2008R2 and windows 2012R2 updates also staring with "Windows 8", so can't filter this keyword"
...
#        $_.Title -match "language\s" -or  #remove langauge packs
        ...
        $_.title -match "Internet Explorer 10" -or 
...
#        $_.title -match "Taiwan" -or  
        ...
        $_.title -match "Preview of Monthly Quality Rollup" -or     #Remove Preview of Monthly Quality Rollup for Windows
        $_.title -match "Preview of Quality Rollup"     #Remove Preview of Quality Rollup for .NET Framework
        )}
        ...
        #Decline updates released more then 2 years prior to the release of an included service pack
...
#                $updatesToDecline = $updates | where {$_.IsDeclined -eq $false -and $_.ProductTitles -contains $SP.ProductTitles -and $_.CreationDate -lt $SP.CreationDate.Addmonths(-3)}
                $updatesToDecline = $updates | where {$_.IsDeclined -eq $false -and $_.ProductTitles -contains $SP.ProductTitles -and $_.CreationDate -lt $SP.CreationDate.Addyears(-2)}
