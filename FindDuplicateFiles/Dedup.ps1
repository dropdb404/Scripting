function Find-PSOneDuplicateFile
{
  <#
	  ...
	  Find-PSOneDuplicateFile -LiteralPath $Path
      ...
      .EXAMPLE
      Find-PSOneDuplicateFile -Path c:\windows -size 10000000
      find log files in the Windows folder with duplicate content
  ...
  & { 
    ...
    catch
    {
      ...
      Get-ChildItem -Path $Path -File -Recurse -ErrorAction Ignore -Filter $Filter
    }
  } | 
  # EXCLUDE EMPTY FILES:
  ...
  & {
    ...
    process
    {
      ...
      foreach($file in $_)
      {
        ...
        # user LiteralPath to elimnate filename with [] will get error if use Path
        $result = Get-FileHash -LiteralPath $file.FullName -Algorithm SHA1
...
$result = Find-PSOneDuplicateFile -Path $Path -size 1000000 -Filter *.pdf
...
& { foreach($key in $result.Keys)
{
    foreach($file in $result[$key])
    {
        $file |
            ...
            Select-Object Hash, Length, FullName,Name 
    }
}
