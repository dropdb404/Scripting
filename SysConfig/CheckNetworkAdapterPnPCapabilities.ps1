...
#requires -Version 2.0
	...
	Foreach($PhysicalAdapter in $PhysicalAdapters)
	{
		...
		If(Test-Path -Path $KeyPath)
		{
			...
			If($PnPCapabilitiesValue -eq 280 -or $PnPCapabilitiesValue -eq 24)
			{
				echo """$PhysicalAdapterName"" - The option ""Allow the computer to turn off this device to save power"" has been disabled already."
			}Else
            {
			    echo """$PhysicalAdapterName"" Setting the value of properties of PnpCapabilities failed."
			}
		}
		Else
		{
